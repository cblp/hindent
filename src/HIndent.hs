...
reformat :: Config
         -> Maybe [Extension]
         -> ByteString
         -> ExceptT String IO Builder
reformat config _mexts =
  preserveTrailingNewline
    (fmap (mconcat . intersperse "\n") . mapM processBlock . cppSplitBlocks)
  where
    processBlock :: CodeBlock -> ExceptT String IO Builder
    processBlock (Shebang text) = pure $ S.byteString text
    processBlock (CPPDirectives text) = pure $ S.byteString text
    processBlock (HaskellSource text) = do
      let ls = S8.lines text
          prefix = findPrefix ls
          code = unlines' (map (stripPrefix prefix) ls)
      lift (parseModuleFromString "<input>" (UTF8.toString code)) >>= \case
        Right (anns, m) ->
          fmap
            (S.lazyByteString . addPrefix prefix . S.toLazyByteString)
            (prettyPrint config (anns, m))
        Left e -> throwError e
...
-- | Print the module.
prettyPrint :: Config -> (Anns, ParsedSource) -> Either a Builder
...
testAst :: ByteString -> Either String ParsedSource
testAst x =
