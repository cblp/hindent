         ...
         CommentSameLine spn c -> do
           ...
           if col == 0
             ...
             then do
               ...
               space
               writeComment c
         CommentAfterLine spn c
         -- when (i == 0) newline
         -- write comment keeping original indentation
          -> do
           let col = fromIntegral $ srcSpanStartColumn spn - 1
           column col $ writeComment c
         CommentDetached _ c -> writeComment c
         _ -> return ())
    (zip [0 :: Int ..] $ sort comments)
  where
...
exp (Tuple _ boxed exps) = do
  let horVariant = parensB boxed $ inter (write ", ") (map pretty exps)
      verVariant =
        parensB boxed $ prefixedLined "," (map (depend space . pretty) exps)
...
exp (Let _ binds e) =
  depend
    ...
    (do pretty binds
        newline
        indented (-4) (depend (write "in ") (pretty e)))
      ...
      ClsTyFam _ h mkind minj ->
        depend
          ...
          (depend
             (pretty h)
             (depend
                (traverse_ (\kind -> write " :: " >> pretty kind) mkind)
...
typ (TyTuple _ Unboxed types) =
  unboxParens $ inter (write ", ") $ map pretty types
typ x =
  case x of
  ...
  case x of
    ...
    RecDecl _ name fields ->
      depend
        (do pretty name
            space)
        (do depend (write "{") (prefixedLined ", " (map pretty fields))
            write "}")
